...
async function seed() {
	...
	try {
		...
		const flight0Seats = createdSeats.filter(
			(s) => s.flightId === createdFlights[0].id && s.class === "economy",
		);
		const flight2Seats = createdSeats.filter(
			(s) => s.flightId === createdFlights[2].id && s.class === "economy",
		);
		...
		console.log(
			"🎉 Database seeded successfully with Vietnamese Airline Data!",
		);
