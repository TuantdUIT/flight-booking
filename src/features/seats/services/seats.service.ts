...
export const seatsService = {
  ...
  updateSeat: async (id: number, input: SeatUpdateInput): Promise<Result<Seat>> => {
    try {
      ...
      if (input.seat_number !== undefined) updateData.seatNumber = input.seat_number;
  ...
  updateAvailability: async (id: number, isAvailable: boolean): Promise<Result<Seat>> => {
    try {
      const existing = await seatsRepository.findById(id);
      if (!existing) {
        return err(errors.notFound("Seat not found"));
