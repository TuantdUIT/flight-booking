...
export function SelectionSummary({
	...
	const isComplete = requiredSeats
				...
				<CardContent className="py-8 text-center">
					...
					{requiredSeats && (
						<p className="text-gray-400 text-xs mt-2">
							You need to select {requiredSeats} seat
							{requiredSeats > 1 ? "s" : ""}
						</p>
					)}
				</CardContent>
						...
						<div className="flex-1 flex flex-wrap items-center gap-x-6 gap-y-3 lg:gap-x-8 min-w-0">
							...
							<div className="flex items-center gap-2">
								...
								<span className="text-sm font-bold text-emerald-600">
									{formatVND(parseFloat(seat.price))} ₫
								</span>
							</div>
						</div>
			...
			<CardFooter className="flex-col gap-2">
				{requiredSeats && selectedSeats.length < requiredSeats && (
					<p className="text-xs text-amber-600 text-center w-full">
						Please select {requiredSeats - selectedSeats.length} more seat
